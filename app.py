...
def rule_extract_locations(text):
    if not text: return ""
    # 去除括号内容、数字、特殊符号，保留中文字符和空格
    clean = re.sub(r'\(.*?\)|（.*?）|\d+\.?\d*|[^\u4e00-\u9fa5\s]', ' ', text)
    # 过滤常见业务干扰词
    for word in ['接', '送', '前往', '返程', '车程', '住', '公里', '行程', '第一天', '第二天', '第三天', '上午', '下午', '晚上']:
...
# --- 主界面 ---
st.markdown('<div class="main-header"><h1>🚌 九江祥隆旅游运输报价系统</h1><p>旗舰版 | 文本提取逻辑修复优化</p></div>', unsafe_allow_html=True)
...
with c1:
    ...
    # 【修复重点1】：添加 key="ocr_raw"，这样用户手动输入的内容会直接更新 session_state['ocr_raw']
    st.text_area("OCR识别结果（可手动输入）：", key="ocr_raw", height=100)

with c2:
    st.markdown("<br>", unsafe_allow_html=True)
    # 提取操作现在会直接读取更新后的 session_state['ocr_raw']
...
with c3:
    # 【修复重点2】：预览框也绑定 key，确保实时编辑生效
    st.text_area("🖊️ 提取地名列表 (可修改)", key="sites_preview", height=80)
    
    if st.button("✅ 确认并同步到下方站点", type="primary", use_container_width=True):
        names = re.split(r'[，,\s]+', st.session_state['sites_preview'])
